...
| Pokemod | Lite（免費版） | Pro（贊助版）：US$7／月，約 NT$226，最多 3 台裝置 | [Pokemod 官方網站](https://pokemod.dev/zh) |
| Shungo | 官方目前只列單一付費方案 | €14.99／月，約 NT$556，每份訂閱限 1 台裝置 | [Shungo 官方網站](https://shungo.app/) |
| PolygonX | 目前主要為付費金鑰 | 透過 Patreon（會員贊助平台）取得 Credits（兌換點數），再兌換需要的金鑰 | [Evermore Labs 官方 Patreon](https://www.patreon.com/evermorelabs) |
...
- 中文協助：前往 `chinese`（中文）頻道找 `GilbertMike`，或聯絡官方列出的 Global Admin／Supporter（全域管理員／支援人員）
...
| 方案 | 官方費用 | 約新台幣 | 主要功能 |
|---|---:|---|---|
| PaC（自動捕捉） | 3 Coins／30 天（US$3） | 約 NT$97 | 自動捕捉、轉補給站、背包與寶可夢整理等自動化功能 |
| PaS 試用（狙擊試用版） | 5 Coins／7 天（US$5） | 約 NT$162 | 自動狙擊、色違獵人、依條件篩選與捕捉寶可夢 |
| PaS（狙擊完整版） | 12 Coins／30 天（US$12） | 約 NT$388 | PaS 完整版 |
| PaC + PaS（捕捉＋狙擊） | 13 Coins／30 天（US$13） | 約 NT$420 | 同時使用 PaC 與 PaS |
| PaH（自動孵蛋） | 1 Coin／30 天（US$1） | 約 NT$32 | Hatching Machine（孵蛋機器），自動孵蛋相關功能 |
| PaK（變隱龍搜尋） | 3 Coins／30 天（US$3） | 約 NT$97 | Kecleon Hunter（變隱龍獵人），變隱龍搜尋相關功能 |
...
7. `GilbertMike` 的官方清單目前列出 Alipay（支付寶）、WeChat（微信）、PayMe、Ko-fi（贊助平台）與 Skrill（電子支付）；實際可用方式仍以購買當下回覆為準。
...
| 方案 | 費用 | 裝置數 | 說明 |
|---|---:|---:|---|
| Lite（免費版） | 免費 | 依官方帳號限制 | 提供主要手動強化與便利功能 |
| Pro（贊助版） | US$7／月（約 NT$226） | 最多 3 台 | 包含 Lite 功能、Pro 專屬功能、最新版本與 Discord Pro 身分組 |

Patreon 結帳時可能依所在地加上稅金或顯示換算後的當地幣別，以結帳頁面為準。

### Lite（免費版）主要功能

- Shiny Scanner（色違掃描器）：掃描色違、IV（個體值）、0 IV（零個體值）、XXL／XXS（特大／特小）、Ditto（百變怪）等目標。
- vPGP²⁺（虛擬 GO Plus）：模擬 GO Plus／Gotcha，自動捕捉與自動轉牌。
- Quest Tweaks（任務調整）：顯示隱藏任務獎勵並自動領取。
- Magic Bag（自動背包整理）：依設定自動刪除多餘道具。
- Tamagotchi（夥伴照顧）：自動照顧夥伴。
- Cooldown Tracker（冷卻計時器）：記錄冷卻操作並避免冷卻期間誤觸。
- Raid Pack（團體戰工具包）：跳過部分團體戰動畫並自動攻擊、閃避。
- Tap to Teleport（點擊傳送）：在地圖上點選位置傳送，需搭配 App Ninjas GPS Joystick（GPS 定位搖桿）。
- Instant Loading（快速載入）、Transfer on Catch（捕捉後自動傳送）、Mass Gift（批次送禮）、Eggspresso（自動孵蛋）。
- Pinap Master（凰梨果自動化）、Performance Mode（效能模式）、Headshot（頭部瞄準輔助）、PvP Metrics（玩家對戰數據）、Incognito Mode（隱私模式）。

### Pro（贊助版）額外功能

- Account Manager（帳號管理）：快速儲存與切換多個 Pokémon GO 帳號。
- Perfect Throw（完美投擲）、Instant Catch（立即捕捉）。
- Inventory Tweaks（背包調整）：批量進化、批量淨化與更多背包操作。
- Swift Gift（快速送禮）、Speed Mode（加速模式）。
- Block Non-Shiny Encounters（阻擋非色違遭遇）、Instant Spin（立即轉牌）。
- Boss Movesets（頭目招式組）與更多 IV 顯示功能。
- Skip Encounter Intro（略過遭遇開場）。
- Integrated Mock Locations Patch（內建模擬定位修補）。
- Spawn Booster（出現範圍強化）。
...
> 官網另列有 Bitcoin（比特幣）「支持我們」地址，但該區塊不等同於自動開通 Pro。若要取得 Pro，請使用明確標示的訂閱或替代付款流程。
...
- Raid（團體戰模式）：依城市、自訂座標與範圍自動進行一般、Dynamax／Gigantamax（極巨化／超極巨化）團體戰並捕捉目標。
- Blatant（高速農怪模式）：高速捕捉、轉補給站、擊敗火箭隊成員並累積經驗值及其他數據。
- Manager（管理模式）：禮物、夥伴、治療、背包及寶可夢整理。
- Kecleon（變隱龍模式）：搜尋變隱龍相關功能。
...
- Shungo 內建定位功能；Blatant（高速農怪）模式若要依路線移動，官方仍建議搭配 Joystick（定位搖桿）。
...
4. 透過官方導向的 Stripe（線上金流平台）結帳頁完成付款。
...
PolygonX 透過 Evermore Labs 的 Patreon（會員贊助平台）提供 Credits（兌換點數），再使用點數取得 PolygonX 金鑰。以下為 2026 年 7 月 23 日官方頁面顯示的 EUR（歐元）月費，結帳時另加 VAT（加值型營業稅）：
...
| Gold Tier（金級） | €26.99（約 NT$1,000） | 30 |
...
|---|---:|---|---|
| Farmer Lite（農怪器輕量版） | 5 | 基本自動農怪與重複動作自動化 | Wood Tier（木材級） |
| Farmer Standard（農怪器標準版） | 10 | 農怪器進階方案 | Stone Tier（石材級） |
| Farmer Premium（農怪器高級版） | 30 | 更多自動化與過濾功能 | Gold Tier（金級） |
| Farmer Platinum（農怪器白金版） | 60 | 高階農怪器方案 | Emerald Tier（綠寶石級） |
| Farmer Titanium（農怪器鈦金版） | 140 | 最高階農怪器方案 | Diamond Tier（鑽石級，150 Credits） |
| Sniper（狙擊器） | 10 | 狙擊與目標篩選 | Stone Tier（石材級） |
| Purifier（淨化器） | 10 | 火箭隊與淨化相關自動化 | Stone Tier（石材級） |
| Battler（戰鬥器） | 15 | 團體戰、Dmax／Gmax（極巨化／超極巨化）戰鬥相關自動化 | Redstone Tier（紅石級） |

> Farmer（農怪器）各等級實際開放的功能邊界沒有完整列在官方公開 Patreon 頁面。請在付款前查看 PolygonX Discord 的最新價格與功能對照，不要只依照上表直接購買。
...
- 自動旋轉補給站。
- 自動進行火箭隊戰鬥。
- 自動進行傳說團體戰、Dmax 與 Gmax（極巨化與超極巨化）戰鬥。
- 可依方案使用目標篩選、行動優先順序、地理圍欄與其他 Farmer（農怪器）自動化功能。
- 官方明確表示不會加入道館戰與 PvP（玩家對戰）自動戰鬥等會直接影響其他玩家的功能。

### 購買步驟

1. 先確認裝置環境可以正常啟動 PolygonX 與官方 Pokémon GO。
2. 前往 [Evermore Labs 官方 Patreon](https://www.patreon.com/evermorelabs)。
3. 依需要的金鑰點數選擇足夠的 Patreon 等級，確認 EUR（歐元）金額、VAT（加值型營業稅）與每月續訂條件。

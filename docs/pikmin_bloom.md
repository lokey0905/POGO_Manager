...
<a id="Defit"></a>
## <img width="24" src="https://raw.githubusercontent.com/lokey0905/POGO_Manager/master/docs/images/icon/defit_icon.png" alt="Defit 圖示" /> Defit
可背景增加走路步數
...
5. 勾選【同步模式】後，系統約5~15分鐘會自動同步步數資訊到Google Fit應用程式中
    ...
    <img width="400" src="https://raw.githubusercontent.com/lokey0905/POGO_Manager/master/docs/images/defit.png" />

6. 皮克敏設定中，將步數設定設定成使用【使用之前的追蹤方式(Google Fit)】

    <img width="400" src="https://raw.githubusercontent.com/lokey0905/POGO_Manager/master/docs/images/pikmin_steps.png" />

### 模式說明

#### 同步模式

勾選【同步模式】後，Defit 會在背景依照設定的速度或步數，定期將健身資料同步到 Google Fit。

- 適合長時間背景累積步數
- 通常約 5～15 分鐘會同步一次資料
- 不需要一直停留在 Defit 畫面
- 可先到 Google Fit 下拉重新整理，確認步數與距離是否更新
- 遊戲端不一定會立即更新，可能需要等待遊戲的冒險同步重新讀取 Google Fit 資料

> 簡單來說：同步模式適合「開著慢慢累積步數」。

#### 隱蔽模式

【隱蔽模式】比較像是一次性的任務模式，需要先設定本次要模擬的距離與速度，開始後等待倒數完成，完成後再同步到 Google Fit。

- 需要選擇本次要增加的距離與速度
- 開始後會進入倒數，倒數期間請保持網路穩定
- 倒數期間建議不要移動手機
- 倒數完成後再到 Google Fit 檢查步數與距離
- 通常不適合頻繁中斷或反覆切換 App

> 簡單來說：隱蔽模式適合「設定一段距離，等待它跑完」。

### 同步模式與隱蔽模式差異

| 項目 | 同步模式 | 隱蔽模式 |
|---|---|---|
| 使用方式 | 開啟後背景持續同步 | 設定距離與速度後等待倒數 |
| 適合情境 | 長時間慢慢累積步數 | 一次指定一段距離 |
| 是否需要等待 | 需要等待同步週期 | 需要等待倒數完成 |
| 是否可背景使用 | 可以背景累積 | 建議等待期間不要移動手機 |
| 是否即時更新 | 不是即時，約數分鐘同步一次 | 倒數完成後再檢查 |
| 速度設定 | 會影響累積速度 | 會影響本次距離完成時間 |

###  速度設定

速度設定通常會影響「每段時間增加多少距離」。

例如：

- 速度越低：距離增加較慢，較接近一般步行
- 速度越高：距離增加較快，較接近快走或慢跑
- 設定過高可能導致遊戲端不容易吃到距離，或資料看起來不自然

建議使用接近正常步行或慢跑的速度，不建議設定過高。

### 使用建議

1. 先確認 Google Fit 內的步數與距離有正常增加
2. 再開啟遊戲確認是否有讀取到資料
3. 如果遊戲沒有立即更新，可等待一段時間後重新開啟遊戲
4. 若長時間無法同步，可嘗試重新登入 Google Fit 與 Defit
5. 確認 Google Fit、健康資料同步平台、Defit 使用的是同一個 Google 帳號
6. 不建議將速度或步數設定過高，避免同步異常或資料不自然

> 注意：Defit 只是將健身資料同步到 Google Fit，遊戲是否採計仍取決於遊戲本身的冒險同步機制。
...
### 步數沒有反映在盆栽背包的花苗上
- 盆栽背包的花苗反映的步數有上限（50000步/天）。如果超過上限，則要到隔天才會反映步數。[官方說明](https://niantic.helpshift.com/hc/faq/3343-faq-about-seedlings/#問：-步數沒有反映在盆栽背包的花苗上。)

### 其他皮克敏問題
- 可參考[官方說明](https://niantic.helpshift.com/hc/zh-hant/23-pikmin-bloom/section/258-troubleshooting/)
